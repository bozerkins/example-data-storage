...
$data = $model->getConnection()->query("
	SELECT SQL_NO_CACHE
		date_table.month as `Request Creation Month`,
		...
		ROUND(SUM(msr_sales) / SUM(msr_requests), 4) as `Sales / Requests`,
		ROUND(SUM(msr_sales) / SUM(msr_sold_requests), 4) as `Sales / Sold Requests`,
		ROUND(SUM(msr_premium_sales) / SUM(msr_requests), 4) as `Premium Sales / Requests`,
		ROUND(SUM(msr_premium_sales) / SUM(msr_sold_requests), 4) as `Premium Sales / Sold Requests`
...
?>
<center>
<h1>Overall request performance</h1>
